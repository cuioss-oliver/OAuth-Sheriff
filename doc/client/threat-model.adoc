...
| Bind the stored session to the `sub` of the validated access token (`StoredToken.subject`). On
  ...
  subject in constant time; a mismatch *refuses the refresh fail-closed* and leaves the stored
  bundle untouched, rather than overwriting it. Where the authorization server also returns a
...
| T-REFRESH-IDENTITY | `TokenDispatcher` refresh mode returns an access token carrying a substituted `sub`; the refresh is refused and the stored bundle asserted untouched. A matched positive control — a refresh whose `sub` matches — is asserted to be accepted and to rotate the bundle, so deleting the guard turns a test red rather than merely leaving one green.
